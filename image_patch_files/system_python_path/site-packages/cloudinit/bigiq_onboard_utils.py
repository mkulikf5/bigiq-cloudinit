...
USERDATA_CONTENT_TYPE = '#cloud-config'
...
def handle_mime(blob):
    # try to parse it as mime-multi part file
    # In case when it's only yaml declaration it will return it
    try:
        mimedata = email.message_from_string(blob)
    except email.Errors.MessageError, val:
        LOG.warning("Userdata multipart MIME parse error: %s", str(val))
        return blob

    if mimedata.is_multipart():
        for partdata in mimedata.get_payload():
            payload = partdata.get_payload()
            if USERDATA_CONTENT_TYPE in payload:
                return payload
        return blob
    else:
        return mimedata.get_payload()

def load_yaml(blob, default=None, allowed=(dict, )):
    loaded = default
    blob = decode_binary(blob)
    blob = handle_mime(blob)
    try:
        LOG.debug(
